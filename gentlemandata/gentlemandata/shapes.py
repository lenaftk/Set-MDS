...
from gentlemandata._fast_utils import distance_matrix, distance_matrix_memmap
...
class Shape(object):
    ...
    def euclidean_distances(self, points=None, use_cache=True):
        ...
        if points is None:
            points = self.points
        if self.memmap == False:
            self.euclidean_d = distance_matrix(points)
            return self.euclidean_d
        else:
            distance_matrix_memmap(points,'./tempfilename')
    ...
    def geodesic_radius(self, points=None, use_cache=True):
        ...
        if self.memmap == False:
            dist = self.euclidean_distances()  ### borw na to souloupwsw me euclidean_distances
        else: 
            self.euclidean_distances()
            dist = np.memmap('./tempfilename', dtype='double', mode='r', shape=(self.num_points,self.num_points))

        nbrs_inc = np.argsort(dist, axis=1)
        max_dist = -1
        fp =  np.memmap('memmap_results',dtype='float64', mode='w+', shape=(self.num_points, self.num_points))
        ...
        if self.memmap == False:
            ...
            fp[:,:] = graph_shortest_path(kng, method='D', directed=False)
            del fp
            return        
...
class SwissRoll(Shape):
    def generate(self, npoints, use_cache=True):
        ...
        self.points, self.color = datasets.make_swiss_roll(
...
class SCurve(Shape):
    def generate(self, npoints, use_cache=True):
        noise_std = 0 if not self.use_noise else self.noise_std
        self.points, self.color = datasets.make_s_curve(
            n_samples=npoints, noise=noise_std, random_state=self.seed)
        return self.points
