...
def compare_elements(dict1,dict2):
    ...
    for key, value in dict1.items():
        if dict1.get(key) == dict2.get(key):
            ...
            false_ +=1
    print("TRUE=", true_, ", FALSE=",false_, "PERCENTAGE= ",true_/(false_+true_),"%" )
...
def run_set_mds(turn,max_iter,radius,radius_barrier,error,points,sample_points,d_current,d_goal,d_sets,n_samples,sets,xs,n_jobs,radius_update_tolerance,explore_dim_percent,prev_error):
    ...
    return xs, d_current,d_sets,error
...
### initializations and parameters
 ...
 ###
    ...
    for kj in range(k):
        ...
        for count,split_point in enumerate(points_to_try_as_splits):
            ...
            for label in range(n_samples):
                ...
                _,_,_,temp_set_error[label]=run_set_mds(turn,10,radius,radius_barrier,temp_set_error[label],points,sample_points,temp_d_current,d_goal,temp_d_sets,n_samples,sets,xs,n_jobs,radius_update_tolerance,explore_dim_percent,prev_error)
    ...
    # update distance matrix
        ...
        xs, d_current,d_sets,error = run_set_mds(turn,max_iter,radius,radius_barrier,error,points,sample_points,d_current,d_goal,d_sets,n_samples,sets,xs,n_jobs,radius_update_tolerance,explore_dim_percent,prev_error)
